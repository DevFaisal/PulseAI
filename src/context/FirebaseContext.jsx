...
import { signUpUser } from "../utils/services/authService";
...
export const FirebaseProvider = ({ children }) => {
  ...
  const createNewDoctor = async (name, specialty, contact, email, password) => {
    try {
      console.log(email);
      console.log(`hospital/${user.hospitalId}/doctors/${email}`);

      // Check if the doctor already exists
      const doctorRef = doc(
        fireStore,
        `hospital/${user.hospitalId}/doctors/${email}`
      );
      ...
      // Add the new doctor to Firestore
      await addDoc(
        collection(fireStore, `hospital/${user.hospitalId}/doctors`),
        {
          ...
          contact,
          email,
          password, // Assuming you'll handle password separately, usually not stored directly in Firestore
        }
      );

      // Optionally, add the doctor to the 'users' collection
      await addDoc(collection(fireStore, "users"), {
        email,
        role: "doctor",
        hospitalId: user.hospitalId,
